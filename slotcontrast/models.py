...
def build(
    ...
    projector = modules.build_projector(model_config.projector)
    projector2 = modules.build_projector(model_config.projector2)
...
class ObjectCentricModel(pl.LightningModule):
    ...
    def forward(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        slots_attn_mask = processor_output["state_attn_mask"]
        state_attn_mask_reg = processor_output["state_attn_mask_reg"]

        decoder_output = self.decoder(slots, decoder=True)

        feat_recon = decoder_output['reconstruction']
        feat_orig_dec = encoder_output['backbone_features']

        if "loss_cl_dec" in self.loss_config:
            feat_orig_dec = self.projector(feat_orig_dec).detach()
            feat_recon = self.projector(feat_recon)
            encoder_output['feat_orig_dec'] = feat_orig_dec
            encoder_output['feat_recon'] = feat_recon

        grp_mask = processor_output["state_attn_mask"]
        grp_labels = torch.argmax(grp_mask, dim=2) # B T HW

        if "loss_cl_enc" in self.loss_config:
            feat_orig_enc = encoder_output['backbone_features'].clone().detach()
            encoder_output['feat_orig_enc'] = feat_orig_enc
            encoder_output['feat_trainable'] = self.projector2(features)
        ...
        outputs = {
            ...
            "slots_attn_mask": slots_attn_mask,
            "state_attn_mask_reg": state_attn_mask_reg,
        }
    ...
    def compute_loss(self, outputs: Dict[str, Any], batch_idx: int) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        losses = {}
        for name, loss_fn in self.loss_fns.items():
            ...
            if "loss_reg" in name:
                lo_redun_start = self.loss_config[name]["train_start"]
                lo_redun_end = self.loss_config[name]["train_end"]
                if batch_idx < lo_redun_start or batch_idx > lo_redun_end:
                    continue
            if name == "loss_cl_dec":
                lo_pcons_start = self.loss_config[name]["train_start"]
                if batch_idx <= lo_pcons_start:
                    continue
                losses[name] = loss_fn(prediction, target, outputs["processor"]["state_attn_mask"])
            elif name == "loss_cl_enc":
                lo_flearn_start = self.loss_config[name]["train_start"]
                if batch_idx <= lo_flearn_start:
                    continue
                losses[name] = loss_fn(prediction, target, outputs["decoder"]["masks"])
            else:
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int):
        ...
        total_loss, losses = self.compute_loss(outputs, batch_idx)
        if len(losses) == 1:
