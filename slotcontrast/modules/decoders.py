...
class MLPDecoder(nn.Module):
    ...
    def forward(self, slots: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # H = W = int(math.sqrt(alpha.shape[2]))
        # alpha_2d = alpha.view(B, T, n_slots, H, W).permute(0, 2, 1, 3, 4)
        # alpha_smooth = alpha_2d.permute(0,2,1,3,4).flatten(0,1)
        #
        # masks = torch.softmax(alpha_smooth.view(bs, n_slots, -1), dim=1)  # (B, S, H*W)
        # masks = masks.view(bs, n_slots, H * W, 1)
