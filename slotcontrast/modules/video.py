...
class LatentProcessor(nn.Module):
    ...
    def forward(
        ...
        if inputs is not None:
            ...
            state_attn_mask_reg = corrector_output['pre_norm_attn4reg'] if 'pre_norm_attn4reg' in corrector_output else None
        else:
            corrector_output = None
            updated_state = state
            state_attn_mask = None
            state_attn_mask4reg = None
...
class MapOverTime(nn.Module):
    ...
    def forward(self, *args, decoder=False):
        batch_size = None
        seq_len = None
        flattened_args = []
