...
@dataclass
class CropParams:
    # pixel 단위 (원본 해상도 336x336 기준)
    y: torch.Tensor  # (B,)
    x: torch.Tensor  # (B,)
    size: torch.Tensor  # (B,)

    # feature grid 단위 (patch=14)
    y_f: torch.Tensor  # (B,)
    x_f: torch.Tensor  # (B,)
    h_f: torch.Tensor  # (B,)
    w_f: torch.Tensor  # (B,)

def _sample_random_crops(B: int,
                         device=None,
                         min_crop: int = 112,
                         max_crop: int = 224,
                         step: int = 14,
                         img_size: int = 336) -> CropParams:
    """
    각 샘플마다 (크기, 위치) 랜덤 샘플링.
    - 크기: [112, 224] 범위, 14의 배수
    - 위치: 14의 배수 격자 위, 이미지 범위 내
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # 가능한 crop size 후보 (14의 배수)
    sizes = torch.arange(min_crop, max_crop + 1, step, device=device)  # e.g., 112,126,...,224 but 우리는 14의 배수만. 위 범위 자체가 14 step
    # 위에서 step=14이니 112,126이 아니라 112,126? -> 112+14=126 맞음. 요구는 "14의 배수" → 112,126,140,...,224 (OK)

    # 샘플별로 size 선택
    size_idx = torch.randint(low=0, high=sizes.numel(), size=(B,), device=device)
    crop_sizes = sizes[size_idx]  # (B,)

    # 각 size에 대해 선택 가능한 y/x 시작 인덱스 개수 (14의 배수 칸)
    # start ∈ {0,14,28,..., img_size - crop_size}  -> 개수 = (img_size - crop_size)/14 + 1
    max_starts = (img_size - crop_sizes) // step  # (B,)
    # 균등 랜덤 정수 [0, max_starts], 이후 *14
    y_slots = torch.floor(torch.rand(B, device=device) * (max_starts + 1)).to(torch.long)
    x_slots = torch.floor(torch.rand(B, device=device) * (max_starts + 1)).to(torch.long)

    y = y_slots * step
    x = x_slots * step

    # feature grid 좌표 (patch=14)
    y_f = y // step
    x_f = x // step
    h_f = crop_sizes // step
    w_f = crop_sizes // step

    return CropParams(y=y, x=x, size=crop_sizes, y_f=y_f, x_f=x_f, h_f=h_f, w_f=w_f)


def _crop_and_resize_per_sample(imgs: torch.Tensor, params: CropParams) -> torch.Tensor:
    """
    imgs: (B,3,336,336), per-sample crop 후 336x336으로 리사이즈하여 반환.
    반환: (B,3,336,336)
    """
    B, C, H, W = imgs.shape
    ...
    for b in range(B):
        y0 = params.y[b].item()
        x0 = params.x[b].item()
        s = params.size[b].item()

        patch = imgs[b:b+1, :, y0:y0+s, x0:x0+s]  # (1,3,s,s)
        patch_resized = F.interpolate(patch, size=(H, W), mode='bilinear', align_corners=False)
        out[b:b+1] = patch_resized
    return out

def _crop_feature_A(A_full: torch.Tensor, params: CropParams) -> torch.Tensor:
    """
    A_full: (B, 24, 24, 384)  # NHWC 가정
    저장된 crop (feature grid 상의 정수 좌표)에 맞춰 crop
    반환: (B, h_f, w_f, 384)  (샘플별 h_f, w_f가 다를 수 있어 list로 묶어 pad/stack? → 여기서는 per-batch max로 pad하지 않고 반환을 list로 관리)
    """
    B, Hf, Wf, C = A_full.shape
    crops = []
    for b in range(B):
        y0f = params.y_f[b].item()
        x0f = params.x_f[b].item()
        hf  = params.h_f[b].item()
        wf  = params.w_f[b].item()
        crops.append(A_full[b:b+1, y0f:y0f+hf, x0f:x0f+wf, :])  # (1,hf,wf,C)
    return crops  # 길이 B의 list, 각 텐서는 (1,hf,wf,C)

def _resize_Aprime_to_match(A_prime: torch.Tensor, params: CropParams):
    """
    A_prime: (B, 24, 24, 384)  # NHWC
    각 샘플의 (h_f, w_f)로 bilinear resize하여 반환 (list)
    결과: 길이 B list, 각 텐서는 (1,hf,wf,C)
    """
    B, Hf, Wf, C = A_prime.shape
    # NHWC -> NCHW (interpolate는 NCHW 기준)
    A_prime_nchw = A_prime.permute(0, 3, 1, 2).contiguous()  # (B,C,Hf,Wf)

    outs = []
    for b in range(B):
        hf = params.h_f[b].item()
        wf = params.w_f[b].item()
        resized = F.interpolate(A_prime_nchw[b:b+1], size=(hf, wf), mode='bilinear', align_corners=False)  # (1,C,hf,wf)
        outs.append(resized.permute(0, 2, 3, 1).contiguous())  # (1,hf,wf,C)
    return outs