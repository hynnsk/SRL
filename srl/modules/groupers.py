...
class SlotAttention(nn.Module):
    ...
    def step(
        self, slots: torch.Tensor, keys: torch.Tensor, values: torch.Tensor, idx: int
        ...
        pre_norm_attn_reg = torch.softmax(dots, dim=-1)
        ...
        return slots, pre_norm_attn, pre_norm_attn_reg
    ...
    def forward(self, slots: torch.Tensor, features: torch.Tensor, n_iters: Optional[int] = None):
        ...
        pre_norm_attn4reg = None
        for idx, _ in enumerate(range(n_iters)):
            slots, pre_norm_attn, pre_norm_attn_reg = self.step(slots, keys, values, idx)
            if idx == 0:
                pre_norm_attn4reg = pre_norm_attn_reg
            else:
                pre_norm_attn4reg = pre_norm_attn4reg + pre_norm_attn_reg
        if pre_norm_attn4reg is not None:
            pre_norm_attn4reg = pre_norm_attn4reg / n_iters

        return {"slots": slots, "masks": pre_norm_attn, "pre_norm_attn4reg": pre_norm_attn4reg}
